...
4. **Server-side authorization always, on every mutation.** Never trust a client-submitted `user_id` or resource ID for ownership — re-derive the authenticated user from the session server-side and check ownership before any write. This is enforced _in addition to_ Supabase Row Level Security, not instead of it — both layers must independently deny unauthorized access. **This is not a redundant belt-and-suspenders rule — confirmed by testing:** `lib/db/postgres-client.ts` connects as the plain `postgres` role (from `DATABASE_URL`), which **bypasses RLS entirely**. RLS only actually applies to direct client access via Supabase's Data API (anon/authenticated roles) — for every Route Handler that reads/writes through `postgres-client.ts`, the server-side check in application code (e.g. `app/api/ads/[id]/route.ts`'s manual `status !== 'active' && !isOwner` check) is the _only_ thing enforcing row-level access control, not a backup to RLS.
